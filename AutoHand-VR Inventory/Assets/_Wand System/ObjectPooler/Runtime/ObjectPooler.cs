...
namespace ObjectPoolingSystem
{
    public class ObjectPooler : MonoBehaviour
    {
        ...
        {
            for (int i = prefabs.poolPrefabs.Count-1; i >= 0; i--)
            {
                ObjectPoolPrefabList.PoolPrefabs pair = prefabs.poolPrefabs[i];
                if (!pooledObjects.ContainsKey(pair.prefab.name))
                {
                    pooledObjects.Add(pair.prefab.name, new Queue<GameObject>());
                }
                GameObject temp;
                for (int j = 0; j < pair.frequency; j++)
                {
        ...
        public GameObject Instantiate(GameObject prefab, Vector3? position = null, Quaternion? rotation = null, Transform parent = null)
        {
            ...
            if (pooledObjects.ContainsKey(prefab.name))
            {
                if (pooledObjects[prefab.name].Count > 0)
                {
                    ...
                    {
                        Debug.LogWarning("Error occured while processing object of type " + prefab.name + ". The queue for this type contains " + pooledObjects[prefab.name].Count.ToString() + " instances. Error: " + e.Message);
                    }
